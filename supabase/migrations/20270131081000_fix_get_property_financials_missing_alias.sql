-- Fix get_property_financials: missing FROM-clause alias (tl) in classified CTE
-- Also keeps AR exclusion and limits bank lines to the property's operating_bank_gl_account_id
    ...
    raw_lines as (
        ...
        where tl.date <= p_as_of
          and (
            ...
            or (
              v_operating_bank_gl_account_id is not null
              and tl.gl_account_id = v_operating_bank_gl_account_id
            )
          )
    ),
    classified as (
...
Excludes accounts receivable lines and limits bank lines to the property''s operating_bank_gl_account_id.';
