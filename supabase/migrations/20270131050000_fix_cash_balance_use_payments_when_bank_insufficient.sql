-- Fix cash balance calculation to use payments_total when bank lines are insufficient
-- If bank lines exist but payments_total is much larger, prefer payments_total
-- This handles cases where payment transactions don't have corresponding bank GL lines yet
...
begin
    ...
    rolled as (
      select
        -- Fix: Use payments_total when bank lines are insufficient (few lines or small total compared to payments)
        -- Per finance model: "if any bank lines → use bankSigned; else use summed payment transactions"
        -- But if bank lines are clearly incomplete (very few lines or total much smaller than payments), prefer payments
        -- IMPORTANT: If we have bank lines AND they're posted to the property's configured bank GL accounts,
        -- always prefer bank lines over payments_total to ensure accurate cash balance
        case
          -- Always use bank_total if we have bank lines (they represent actual bank account transactions)
