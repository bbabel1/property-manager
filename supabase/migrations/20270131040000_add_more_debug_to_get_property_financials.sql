-- Add more debug information to get_property_financials to investigate cash balance calculation
-- This will show debits/credits breakdown and line counts
...
begin
    ...
    rolled as (
      select
        case
          -- Use bank_total if there are bank lines (count > 0), not just if total <> 0
          -- This matches finance model: "if any bank lines → use bankSigned; else use summed payment transactions"
