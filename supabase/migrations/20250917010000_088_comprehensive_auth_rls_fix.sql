...
BEGIN
  IF EXISTS (
    SELECT 1 FROM information_schema.tables
    WHERE table_schema = 'public' AND table_name = 'tenant_notes'
  ) THEN
    EXECUTE 'DROP POLICY IF EXISTS "tenant_notes_read_policy" ON public.tenant_notes';
...
BEGIN
  IF EXISTS (
    ...
    WHERE routine_schema = 'public' AND routine_name = 'enforce_data_integrity'
  ) THEN
    EXECUTE 'DROP FUNCTION IF EXISTS public.enforce_data_integrity()';
...
BEGIN
  IF EXISTS (
    SELECT 1 FROM information_schema.routines
    WHERE routine_schema = 'public' AND routine_name = 'jwt_custom_claims'
  ) THEN
    EXECUTE 'DROP FUNCTION IF EXISTS public.jwt_custom_claims()';
