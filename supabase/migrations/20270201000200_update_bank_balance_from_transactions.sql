-- Align bank balances with transactions.bank_gl_account_id (deposits minus payments).
-- - gl_account_balance_as_of now includes transaction headers that point at a bank GL.
-- - v_gl_account_balances_as_of folds in those transaction totals for global balances.
...
'Returns debit-minus-credit balance for a GL account up to an as-of date, org-scoped. Includes transaction_lines + transactions.bank_gl_account_id (deposits minus payments). When property_id is provided, includes linked lines for that property.';
...
'Org-scoped GL balances as-of date. Global rows include transaction_lines plus transactions.bank_gl_account_id (deposits minus payments). Property rows remain transaction_line scoped.';
