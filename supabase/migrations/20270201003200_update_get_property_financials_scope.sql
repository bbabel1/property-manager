-- Align get_property_financials to honor transaction header property/unit scope
...
begin
    ...
    raw_lines as (
        ...
        where tl.date <= p_as_of
          and (
            ...
            or t.unit_id in (select id from property_units)
            or t.lease_id in (select id from property_leases)
            or t.buildium_lease_id in (select buildium_lease_id from property_leases where buildium_lease_id is not null)
            or tl.property_id = p_property_id
    ...
    payment_tx as (
      select coalesce(sum(abs(coalesce(t.total_amount,0))) filter (where lower(coalesce(t.transaction_type,'')) similar to '%(payment|credit|refund|adjustment|receipt)%'),0) as payments_total
      from public.transactions t
      where (t.lease_id in (select id from property_leases) or t.buildium_lease_id in (select buildium_lease_id from property_leases where buildium_lease_id is not null))
        and t.date <= p_as_of
    ),
    payment_totals as (
      select
    ...
    select jsonb_build_object(
      ...
      'available_balance',
        ...
        + (case when pt.deposits_from_payments <> 0 then pt.deposits_from_payments else lt.deposit_total end)
        + (case when pt.prepayments_from_payments <> 0 then pt.prepay_total end)
        - v_reserve,
      'as_of', p_as_of
    )
...
'Returns aggregate property financial metrics as of a given date, using transaction header scope (property/unit/lease) with fallbacks to line scope for legacy data.';
