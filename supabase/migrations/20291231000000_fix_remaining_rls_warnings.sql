  ...
  IF EXISTS (
    ...
    CREATE POLICY lease_org_write ON public.lease
      FOR INSERT WITH CHECK (
        public.is_org_admin_or_manager((select auth.uid()), lease.org_id)
      ...
      FOR SELECT USING (
        EXISTS (
          ...
          WHERE mr.role_id = role_permissions.role_id
            AND public.is_org_member((select auth.uid()), mr.org_id)
        )
      );
    ...
    CREATE POLICY permission_profile_permissions_write ON public.role_permissions
      FOR INSERT WITH CHECK (
        EXISTS (
          ...
          WHERE mr.role_id = role_permissions.role_id
            AND (
              public.is_org_admin((select auth.uid()), mr.org_id)
              OR public.is_platform_admin((select auth.uid()))
            )
        )
      );
...
BEGIN
  IF EXISTS (
    ...
    CREATE POLICY membership_roles_admin_write ON public.membership_roles
      ...
      USING (
        public.is_org_admin((select auth.uid()), membership_roles.org_id)
        OR public.is_platform_admin((select auth.uid()))
      )
      WITH CHECK (
        public.is_org_admin((select auth.uid()), membership_roles.org_id)
