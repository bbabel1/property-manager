-- Protect lease creation functions from legacy file_links schema references
-- When the unified files table is present (no source/file_links), skip document inserts
...
DECLARE
  ...
  v_property_id uuid;
  v_unit_id uuid;
  v_legacy_files_schema boolean := false;
BEGIN
  ...
  -- Derive org from property/unit when not supplied in payload
  ...
  -- Documents metadata: only run when legacy file_links schema is still present
  SELECT EXISTS (
    SELECT 1
  ...
  ) INTO v_legacy_files_schema;

  IF v_legacy_files_schema THEN
    WITH docs AS (
