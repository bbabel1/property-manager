-- Fix function signature conflicts and return statement issues
-- 1. Drop old 4-parameter version of gl_account_balance_as_of to resolve ambiguity
-- 2. Fix get_property_financials return statement

begin;

-- Drop the old 4-parameter version of gl_account_balance_as_of
-- The new 5-parameter version handles 4-parameter calls via defaults
drop function if exists public.gl_account_balance_as_of(uuid, uuid, date, uuid);

-- Fix get_property_financials return statement
...
'Financial snapshot for a property as of date, for UI consumption. Only includes Rental entity type transactions to ensure property cash balances exclude Company-level transactions.';
