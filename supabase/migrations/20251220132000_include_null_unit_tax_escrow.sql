...
begin
  ...
  with transaction_amounts as (
    ...
    left join lateral (
      ...
      order by
        case when v_unit_id is not null and (tl.unit_id = v_unit_id or tl.unit_id is null) then 0 else 1 end,
        case
...
-- Ensure all Tax Escrow accounts are categorized as deposit
DO $$
DECLARE
  v_account_id UUID;
BEGIN
  FOR v_account_id IN
    SELECT ga.id
    FROM gl_accounts ga
    WHERE ga.name ILIKE '%tax escrow%'
      AND NOT EXISTS (
        SELECT 1
