-- Scope get_property_financials cash balance to the property's own lines
-- (or truly unscoped bank lines), avoiding contamination from other properties
-- that share the same bank GL. Also include deposit_trust_gl_account_id in bank GL set.
...
begin
    ...
    property_bank_gls as (
        select array_remove(array[v_operating_bank_gl_account_id, v_deposit_trust_gl_account_id], null) as ids
    ),
    raw_lines as (
        select
          tl.*,
          ga.name as ga_name,
          ga.type as ga_type,
          ga.sub_type as ga_sub_type,
          ga.is_bank_account as ga_is_bank,
          ga.is_security_deposit_liability as ga_is_sdl,
          coalesce(ga.exclude_from_cash_balances, false) as ga_exclude,
          pbg.ids as bank_gl_ids
        from public.transaction_lines tl
        join public.gl_accounts ga on ga.id = tl.gl_account_id
        cross join property_bank_gls pbg
        where tl.date <= p_as_of
          and (
            ...
            or (
              array_length(pbg.ids, 1) is not null
              and tl.gl_account_id = any(pbg.ids)
              and (
                tl.property_id = p_property_id
                or tl.unit_id in (select id from property_units)
                or tl.lease_id in (select id from property_leases)
                ...
                or (
                  tl.property_id is null
                  and tl.unit_id is null
                  and tl.lease_id is null
                  and tl.buildium_lease_id is null
                )
              )
            )
          )
    ...
    classified as (
        select
          ...
          case
            ...
            when array_length(rl.bank_gl_ids, 1) is not null and rl.gl_account_id = any(rl.bank_gl_ids) then true
            else false
          end as bank_flag,
          (rl.ga_is_sdl = true
...
'Returns property financials with cash balance calculated from bank GL transaction_lines scoped to the property/unit/lease (or unscoped bank lines). Prevents shared bank GL lines from other properties from inflating balances, and includes deposit_trust_gl_account_id in the bank GL set.';
