...
async function buildium<T>(
  ...
  if (!res.ok) {
    ...
    throw new Error(
      `${res.status} ${res.statusText} - ${err?.Message || err?.message || 'Buildium error'}`,
    );
  }
...
async function resolveGLAccountId(
  ...
  const insert = {
    ...
    sub_accounts: Array.isArray(acc?.SubAccounts)
      ? acc.SubAccounts.map((s: any) => s?.Id).filter(Boolean)
      : null,
    created_at: now,
    updated_at: now,
  };
...
async function resolveLeaseWithOrg(
  supabase: any,
  buildiumLeaseId?: number | null,
): Promise<{ id: number | null; org_id: string | null }> {
...
function mapHeaderFromBuildium(tx: any) {
  const payeeTenantId = tx?.PayeeTenantId ?? tx?.PayeeTenantID ?? tx?.Payee?.TenantId ?? null;
  return {
  ...
  try {
    ...
    if (!orgId) {
      return new Response(
        JSON.stringify({
          success: false,
          error: 'orgId required for Buildium lease transaction API',
        }),
        { headers, status: 400 },
      );
    }
    if (!currentBuildiumCreds.clientId || !currentBuildiumCreds.clientSecret) {
      return new Response(
        JSON.stringify({ success: false, error: 'Missing Buildium credentials' }),
        { headers, status: 400 },
      );
    }
    ...
    switch (action) {
      ...
      case 'getRecurring': {
        data = await buildium<any>(
          ...
          orgId,
          'GET',
          `/leases/${leaseId}/recurringtransactions/${recurringId}`,
        );
        break;
      }
