...
function v1TaskStatusToLocal(status: string | null | undefined): string {
  switch ((status || '').toLowerCase()) {
    case 'new': return 'open'; case 'inprogress': case 'in_progress': return 'in_progress'; case 'completed': return 'completed'; case 'cancelled': return 'cancelled'; case 'onhold': case 'on_hold': return 'on_hold'; default: return 'open'
  }
}
...
async function resolveLeaseId(buildiumLeaseId: number | null | undefined, supabase: any): Promise<number | null> { if (!buildiumLeaseId) return null; const { data } = await supabase.from('lease').select('id').eq('buildium_lease_id', buildiumLeaseId).maybeSingle(); return data?.id ?? null }
async function resolveTenantId(buildiumTenantId: number | null | undefined, supabase: any): Promise<string | null> { if (!buildiumTenantId) return null; const { data } = await supabase.from('tenants').select('id').eq('buildium_tenant_id', buildiumTenantId).maybeSingle(); return data?.id ?? null }
async function contactIdForTenant(tenantId: string | null, supabase: any): Promise<number | null> { if (!tenantId) return null; const { data } = await supabase.from('tenants').select('contact_id').eq('id', tenantId).maybeSingle(); return data?.contact_id ?? null }
...
async function upsertTaskFromBuildium(item: any, supabase: any) {
  ...
  const leaseId = await resolveLeaseId(item?.LeaseId, supabase)
  const tenantLocalId = await resolveTenantId(item?.TenantId, supabase)
  const requestedByContactId = await contactIdForTenant(tenantLocalId, supabase)
  ...
  const row = {
    ...
    lease_id: leaseId,
    tenant_id: tenantLocalId,
    requested_by_contact_id: requestedByContactId,
    requested_by_type: 'Resident',
    requested_by_buildium_id: typeof item?.TenantId === 'number' ? item.TenantId : null,
    task_kind: 'resident' as const,
    ...
    buildium_tenant_id: typeof item?.TenantId === 'number' ? item.TenantId : null,
    buildium_lease_id: typeof item?.LeaseId === 'number' ? item.LeaseId : null,
