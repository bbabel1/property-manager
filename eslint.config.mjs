...
const eslintConfig = [
    ...
    rules: {
          ...
          patterns: [
            {
              ...
              message:
                '⚠️ Use enhanced mappers (mapPropertyFromBuildiumWithBankAccount, mapBankAccountFromBuildiumWithGLAccount, mapGLAccountFromBuildiumWithSubAccounts) to ensure proper relationship handling',
            },
          ],
        },
      ],
      ...
      'no-restricted-globals': [
        ...
        },
        {
          ...
          message:
            '⚠️ Use mapBankAccountFromBuildiumWithGLAccount() instead to ensure proper GL account relationship handling',
        },
        {
          name: 'mapGLAccountFromBuildium',
          message:
            '⚠️ Use mapGLAccountFromBuildiumWithSubAccounts() instead to ensure proper sub_accounts relationship handling',
        },
      ],
      ...
      'no-restricted-syntax': [
        'error',
        {
          selector: 'Literal[value=/palette-[a-z-]+/]',
          message:
            'Use tokenized Tailwind utilities (e.g., text-primary, bg-muted, border-success-500) instead of palette-* helpers.',
        },
        {
          selector: 'TemplateElement[value.raw=/palette-[a-z-]+/]',
          message:
            'Use tokenized Tailwind utilities (e.g., text-primary, bg-muted, border-success-500) instead of palette-* helpers.',
        },
        {
          selector: 'Literal[value=/\\b(text|bg|border)-\\[\\s*var\\(--color-[^)]+\\)\\s*\\]/]',
          message:
            'Avoid inline color variables; add a token in styles/tokens.css, map it in tailwind.config.ts, and use the Tailwind utility instead of text-[var(--color-…)]/bg-[…]/border-[…].',
        },
        {
          selector:
            'TemplateElement[value.raw=/\\b(text|bg|border)-\\[\\s*var\\(--color-[^)]+\\)\\s*\\]/]',
          message:
            'Avoid inline color variables; add a token in styles/tokens.css, map it in tailwind.config.ts, and use the Tailwind utility instead of text-[var(--color-…)]/bg-[…]/border-[…].',
        },
      ],
  ...
  // Tooling and config files run in Node; relax browser/React-centric rules
  {
    ...
    rules: {
      ...
      // Allow config files to declare lint patterns without tripping the same rules
