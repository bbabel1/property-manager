...
- **Create/Sync**: `create-*`, `import-buildium.ts`, `manual-lease-sync.ts`, `map-buildium-rent-id.ts` for targeted data
...
- **Backup first**: many scripts mutate production data; capture a backup or run against a clone before applying
  broadly.
