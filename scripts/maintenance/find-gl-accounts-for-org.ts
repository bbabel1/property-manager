...
async function main() {
  ...
  const arCandidates = assetAccounts.filter(a => 
    a.is_active && 
  ...
  const rentExact = incomeAccounts.find(a => 
    a.is_active && a.name?.toLowerCase() === 'rent income'
  )
  const rentCandidates = incomeAccounts.filter(a => 
    a.is_active && 
    a.name?.toLowerCase().includes('rent')
  )
  const bestRent = rentExact || rentCandidates[0] || incomeAccounts.find(a => a.is_active) || incomeAccounts[0]
  if (bestRent) {
  ...
  const lateFeeCandidates = incomeAccounts.filter(a => 
    a.is_active && 
    a.name?.toLowerCase().includes('late')
  )
  ...
  const undepositedCandidates = assetAccounts.filter(a => 
    a.is_active && 
    (a.name?.toLowerCase().includes('undeposited') || 
     a.name?.toLowerCase().includes('cash'))
  )
  if (undepositedCandidates.length > 0) {
    console.log(`  Undeposited Funds: ${undepositedCandidates[0].name} (${undepositedCandidates[0].id})`)
  }
