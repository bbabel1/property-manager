-- Phase 1 core: enums, columns, helper tables, aggregate function

-- 1) Sync status enum on leases
DO $$ BEGIN
  CREATE TYPE public.sync_status_enum AS ENUM ('pending','ok','error');
EXCEPTION WHEN duplicate_object THEN NULL; END $$;

ALTER TABLE public.lease
  ADD COLUMN IF NOT EXISTS sync_status public.sync_status_enum DEFAULT 'pending',
...
-- 3) Lease sync queue (for Buildium retries)
DO $$ BEGIN
  CREATE TABLE public.lease_sync_queue (
...
-- 4) Aggregate create function (single TX)
