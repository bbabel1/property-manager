...
async function testUnitDetailsLogic() {
  try {
    console.log('🧪 Testing Unit Details Logic (Exact Copy)...\n')

    // First, let's get a unit with an active lease
    ...
    console.log(`🏠 Testing with Unit: ${testUnit.unit_number} (${testUnit.id})`)
    ...
    console.log(`📋 Found ${leases.length} leases for unit`)
