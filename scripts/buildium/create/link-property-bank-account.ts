...
/**
 * Link Property 7647's operating bank account to the local bank account record
 */
...
// Fetch property from Buildium to get the operating bank account ID
...
// Update property with bank account link
async function updatePropertyBankAccount(propertyId: string, bankAccountId: string) {
  logger.info(`Updating property ${propertyId} with bank account ${bankAccountId}`)
  
  const { data: updatedProperty, error } = await supabase
    .from('properties')
    .update({
      operating_bank_account_id: bankAccountId,
  ...
  logger.success(`Successfully updated property with bank account link`)
  return updatedProperty
}

// Main function
async function linkPropertyBankAccount() {
  try {
    const propertyId = 7647
    
    logger.info('Starting property bank account linking process...')
    ...
    // 1. Fetch property from Buildium to get operating bank account ID
    ...
    if (!buildiumProperty.OperatingBankAccountId) {
      logger.info('Property does not have an operating bank account ID')
      return null
    }
    
    // 2. Find the corresponding bank account in local database
    const bankAccount = await findBankAccountByBuildiumId(buildiumProperty.OperatingBankAccountId)
    ...
    // 4. Update property with bank account link
    const updatedProperty = await updatePropertyBankAccount(localProperty.id, bankAccount.id)
    
    logger.success('Property bank account linking completed successfully!')
    ...
    logger.error('Property bank account linking failed:', error)
...
export { linkPropertyBankAccount }
