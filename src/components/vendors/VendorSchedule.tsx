    ...
    <Card>
      <CardHeader className="flex flex-col gap-2">
        <div className="flex items-center justify-between">
          <div>
            <CardTitle headingSize="h5">Scheduling & site visits</CardTitle>
        ...
        <div className="space-y-3">
          ...
          <div className="space-y-3">
            {upcoming.length > 0 ? (
              upcoming.map((item) => (
                <div key={item.workOrderId} className="rounded-lg border border-border/70 bg-muted/40 p-3">
                  <div className="flex items-center justify-between gap-2">
                    <div>
                      <Label as="p">{item.vendorName}</Label>
                      <Body as="p" size="xs" tone="muted">{item.subject}</Body>
                    </div>
                    {item.priority ? <Badge variant="outline" className="uppercase">{item.priority}</Badge> : null}
                  </div>
                  <Body as="div" size="xs" tone="muted" className="mt-2 flex flex-wrap items-center gap-3">
              ...
              <Body as="div" size="sm" tone="muted" className="flex h-[180px] flex-col items-center justify-center gap-2 rounded-lg border border-dashed border-border/60 text-center">
...
function CheckIcon() {
  return <svg viewBox="0 0 20 20" className="h-5 w-5 text-primary-500" aria-hidden="true"><path fill="currentColor" d="M16.707 5.293a1 1 0 0 0-1.414 0L8 12.586 4.707 9.293a1 1 0 0 0-1.414 1.414l4 4a1 1 0 0 0 1.414 0l8-8a1 1 0 0 0 0-1.414Z" /></svg>
}
