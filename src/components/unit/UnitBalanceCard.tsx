...
export default function UnitBalanceCard({ fin, rent }: { fin?: { cash_balance?: number; security_deposits?: number; reserve?: number; available_balance?: number }; rent?: number | null }) {
  ...
  return (
    <Card>
      <CardContent className="p-4 bg-muted rounded-md space-y-1">
        ...
        <div className="flex items-center justify-between text-sm text-muted-foreground"><span>Prepayments:</span><span>{fmt(0)}</span></div>
