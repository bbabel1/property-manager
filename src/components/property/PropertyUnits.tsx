    ...
    <div className="space-y-6">
      ...
      {!isLoading && units.length === 0 && (
        <div className="text-center py-12">
          ...
          <p className="text-gray-500 mb-6">Add units to this property to start managing rentals and leases.</p>
          <Button onClick={() => setShowAddUnitModal(true)}>
            <Plus className="w-4 h-4 mr-2" />
            Add Your First Unit
          </Button>
      ...
      {!isLoading && units.length > 0 && (
        <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6">
          {filteredUnits.map((unit) => (
            <div key={unit.id} className="bg-card rounded-lg border p-6 hover:shadow-md transition-shadow">
