...
export default function AddResidentRequestTaskForm({
  categoryOptions,
  propertyOptions,
  staffOptions,
}: Props) {
  ...
  useEffect(() => {
    if (!formState.residentId) return;
    const matched = residentOptions.find((resident) => resident.value === formState.residentId);
    const propertyId = matched?.meta?.property_id;
    if (propertyId) {
      setFormState((prev) => ({ ...prev, propertyId }));
    }
  }, [formState.residentId, residentOptions]);
  ...
  const handleSubmit = async (event: React.FormEvent<HTMLFormElement>) => {
    ...
    try {
      ...
      toast.success('Resident request created (demo)');
      router.push('/tasks');
    } catch (error) {
      console.error('Failed to create resident task', error);
            ...
            <section className="space-y-4">
              <div className="space-y-2">
                ...
                <Select
                  ...
                  onValueChange={(value) => updateField('residentId', value)}
                  disabled={residentLoading || !!residentError || isSaving}
                >
                  <SelectTrigger>
              ...
              <div className="grid gap-4">
                ...
                <div className="space-y-2">
                  ...
                  <Textarea
                    id="resident-task-description"
                    placeholder="Describe the request or include notes from the resident..."
