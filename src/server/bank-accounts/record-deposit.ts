...
export type RecordDepositPrefillResult =
  | { ok: true; data: RecordDepositPrefill }
  | { ok: false; error: string };

export async function loadRecordDepositPrefill(
  bankAccountId: string,
): Promise<RecordDepositPrefillResult> {
  try {
    const db = supabaseAdmin || supabase;
    if (!db) {
      return { ok: false, error: 'Database client is unavailable.' };
    }
    ...
    if (udfGlAccountId) {
      const dbAny = db as unknown as SupabaseClient<any>;
      const { data: depositItemsRows } = await dbAny
        .from('deposit_items')
        .select('payment_transaction_id, buildium_payment_transaction_id')
        .limit(10000);

      (depositItemsRows || []).forEach((row: { payment_transaction_id?: string | null; buildium_payment_transaction_id?: number | null }) => {
        if (row?.payment_transaction_id) depositedPaymentIds.add(String(row.payment_transaction_id));
        const idNum = Number(row?.buildium_payment_transaction_id ?? NaN);
        if (Number.isFinite(idNum)) depositedBuildiumPaymentIds.add(idNum);
      });

      // Legacy fallback: Buildium payment IDs stored on transaction_payment_transactions
    ...
    if (udfGlAccountId) {
      const { data: txRowsData } = await db
        ...
        .in('transaction_type', ['Payment', 'ElectronicFundsTransfer', 'ApplyDeposit', 'RentalApplicationFeePayment'])
      ...
      for (const row of candidates) {
        ...
        if (row?.id && depositedPaymentIds.has(String(row.id))) continue;
        ...
        if (allowedBankIds.size > 0 && !allowedBankIds.has(bankAccountId)) continue;
    ...
    const data: RecordDepositPrefill = {
      bankAccountId,
      ...
      units,
      glAccounts,
      depositStatuses: DEPOSIT_STATUSES,
      defaultDepositStatus: 'posted',
    };

    return { ok: true, data };
  } catch (error) {
    console.error('Failed to load record deposit prefill', error);
    return { ok: false, error: 'Unable to load record deposit data.' };
  }
}
