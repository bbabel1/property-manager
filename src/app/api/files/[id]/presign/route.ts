...
export async function GET(request: NextRequest, { params }: { params: Promise<{ id: string }> }) {
  try {
    ...
    if (file.storage_provider === 'buildium') {
      ...
      const res = await buildiumFetch('POST', `/files/${id}/download`, undefined, undefined, orgId ?? undefined)
    ...
    return NextResponse.json({ error: 'Failed to presign file' }, { status: 500 })
  }
}
