...
export async function GET(
  ...
  try {
    ...
    const buildiumUrl = `${process.env.BUILDIUM_BASE_URL}/generalledger/entries/${id}`;
    ...
    if (!response.ok) {
      ...
      logger.error(`Buildium general ledger entry fetch failed`);

      return NextResponse.json(
        { 
          error: 'Failed to fetch general ledger entry from Buildium',
          details: errorData
        },
...
export async function PUT(
  ...
  try {
    ...
    if (!response.ok) {
      const errorData = await response.json().catch(() => ({}));
      logger.error(`Buildium general ledger entry update failed`);

      return NextResponse.json(
        { 
          error: 'Failed to update general ledger entry in Buildium',
          details: errorData
        },
        { status: response.status }
      );
