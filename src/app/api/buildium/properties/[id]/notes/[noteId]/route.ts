...
export async function GET(request: NextRequest, { params }: { params: Promise<{ id: string; noteId: string }> }) {
  try {
    ...
    const response = await buildiumFetch('GET', `/rentals/${id}/notes/${noteId}`, undefined, undefined, undefined);
...
export async function PUT(request: NextRequest, { params }: { params: Promise<{ id: string; noteId: string }> }) {
  try {
    ...
    // Make request to Buildium API
    const response = await buildiumFetch('PUT', `/rentals/${id}/notes/${noteId}`, undefined, validatedData, undefined);
