...
export async function GET(
  ...
  try {
    ...
    const buildiumUrl = `${process.env.BUILDIUM_BASE_URL}/workorders/${id}`;
    ...
    if (!response.ok) {
      ...
      logger.error(`Buildium work order fetch failed`);

      return NextResponse.json(
        { 
          error: 'Failed to fetch work order from Buildium',
          details: errorData
        },
...
export async function PUT(
  ...
  try {
    ...
    if (!response.ok) {
      const errorData = await response.json().catch(() => ({}));
      logger.error(`Buildium work order update failed`);

      return NextResponse.json(
        { 
          error: 'Failed to update work order in Buildium',
          details: errorData
        },
        { status: response.status }
      );
