...
export async function GET(request: NextRequest, { params }: { params: Promise<{ id: string; historyId: string }> }) {
  try {
    ...
    const response = await buildiumFetch('GET', `/tasks/${id}/history/${historyId}`, undefined, undefined, undefined);
...
export async function PUT(request: NextRequest, { params }: { params: Promise<{ id: string; historyId: string }> }) {
  try {
    ...
    // Make request to Buildium API
    const response = await buildiumFetch('PUT', `/tasks/${id}/history/${historyId}`, undefined, validatedData, undefined);
