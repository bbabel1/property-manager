...
export async function GET(request: NextRequest, { params }: { params: Promise<{ categoryId: string }> }) {
  try {
    ...
    const response = await buildiumFetch('GET', `/tasks/categories/${categoryId}`, undefined, undefined, undefined);
...
export async function PUT(request: NextRequest, { params }: { params: Promise<{ categoryId: string }> }) {
  try {
    ...
    // Make request to Buildium API
    const response = await buildiumFetch('PUT', `/tasks/categories/${categoryId}`, undefined, validatedData, undefined);
