...
export async function GET(
  ...
  try {
    ...
    const buildiumUrl = `${process.env.BUILDIUM_BASE_URL}/rentals/units/${id}`;
    ...
    if (!response.ok) {
      ...
      logger.error(`Buildium unit fetch failed`);

      return NextResponse.json(
        { 
          error: 'Failed to fetch unit from Buildium',
          details: errorData
        },
...
export async function PUT(
  ...
  try {
    ...
    if (!response.ok) {
      const errorData = await response.json().catch(() => ({}));
      logger.error(`Buildium unit update failed`);

      return NextResponse.json(
        { 
          error: 'Failed to update unit in Buildium',
          details: errorData
        },
        { status: response.status }
      );
