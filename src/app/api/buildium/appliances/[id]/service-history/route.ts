...
export async function GET(request: NextRequest, { params }: { params: Promise<{ id: string }> }) {
  try {
    ...
    const response = await buildiumFetch('GET', `/rentals/appliances/${id}/servicehistory`, queryParams, undefined, orgId);
...
export async function POST(request: NextRequest, { params }: { params: Promise<{ id: string }> }) {
  try {
    ...
    // Make request to Buildium API
    const response = await buildiumFetch('POST', `/rentals/appliances/${id}/servicehistory`, undefined, validatedData, orgId);
