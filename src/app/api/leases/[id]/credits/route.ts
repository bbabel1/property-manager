...
export async function POST(
  ...
  const supabase = getServerSupabaseClient('credits:create')
  ...
  if (txError || !transaction) {
    return NextResponse.json({ error: txError?.message || 'Failed to issue credit' }, { status: 500 })
  }
  ...
  if (linePayloads.length) {
    const { data: insertedLines, error: lineError } = await supabase
      ...
      .select()
    if (lineError) {
      return NextResponse.json({ error: lineError.message || 'Failed to create credit lines' }, { status: 500 })
    }
    lines = insertedLines ?? []
  }
