...
async function buildKpisFromTables(supabase: TypedSupabaseClient, orgId: string) {
  ...
  const firstError =
    properties.error || rentRollNow.error || rentRollPrev.error || activeLeasesCount.error || workOrders.error
  if (firstError) {
    throw firstError
  }

  const propertyRows = properties.data ?? []
  const propertyIds = propertyRows.map((row) => (row as { id?: string | null }).id).filter(Boolean) as string[]
  const totals = propertyRows.reduce(
    (acc, row) => {
      ...
      const occupied = toNumber((row as { total_occupied_units?: number | null }).total_occupied_units)
  ...
  if (propertyIds.length) {
    ...
    const normalized = (units ?? []).filter((u) => (u as { is_active?: boolean | null }).is_active ?? true)
    const occupied = normalized.filter((u) => (u as { status?: string | null }).status?.toLowerCase() === 'occupied')
      .length
    const vacant = normalized.filter((u) => (u as { status?: string | null }).status?.toLowerCase() === 'vacant').length
    const active = normalized.filter((u) => (u as { status?: string | null }).status?.toLowerCase() !== 'inactive').length
    unitsFromDetail = {
      total: active,
      occupied,
...
export async function GET(req: Request, { params }: { params: Promise<{ orgId: string }> }) {
  try {
    ...
    process.stdout.write(`[Dashboard] Querying expiring leases: orgId=${orgId}, dateFrom=${expiringLeasesDateFrom}, dateTo=${expiringLeasesDateTo}\n`)
    ...
    const [kpis, renewals, onboardingSummary, recentTx, activeWOs, expiringLeases] = await Promise.all([
      ...
      (async () => {
        ...
        if (allActiveLeases.error) {
          process.stdout.write(`[Dashboard] Error fetching active leases: ${allActiveLeases.error.message}\n`)
        }
        if (activeCount > 0 && activeCount <= 5) {
          process.stdout.write(`[Dashboard] Sample leases: ${JSON.stringify(allActiveLeases.data?.map((l) => ({
            id: l.id,
        ...
        process.stdout.write(`[Dashboard] Filtered expiring leases: count=${filteredCount}, dateFrom=${dateFrom}, dateTo=${dateTo}\n`)
        if (result.error) {
          process.stdout.write(`[Dashboard] Filtered query error: ${result.error.message}\n`)
        }
        if (filteredCount > 0) {
          process.stdout.write(`[Dashboard] Expiring leases: ${JSON.stringify(result.data?.slice(0, 3))}\n`)
        }
    ...
    if (error) {
      logger.error({ error, orgId, expiringLeasesError: expiringLeases.error }, 'Dashboard API error')
    ...
    expiringLeaseRows.forEach((lease) => {
      ...
      const daysUntil = Math.floor((leaseToStartOfDay.getTime() - startOfTodayForCalc.getTime()) / (1000 * 60 * 60 * 24))
    ...
    // Final debug output
    process.stdout.write(`[Dashboard] Final expiring leases buckets: ${JSON.stringify(bucketDefs.map((bucket) => ({
      key: bucket.key,
