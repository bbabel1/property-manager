...
export async function GET(request: NextRequest) {
  try {
    ...
    for (const row of (data || []) as PropertyRow[]) {
      ...
      if (!buildingMap.has(building.id)) {
        buildingMap.set(building.id, {
          ...
          borough: boroughFromCode(building.borough_code),
          ...
          dwellingUnitCount: building.dwelling_unit_count,
          totalUnits: 0,
          occupiedUnits: 0,
          vacantUnits: 0,
          occupancyRate: 0,
    ...
    const payload = Array.from(buildingMap.values()).map((summary) => {
      if (!summary.totalUnits && summary.dwellingUnitCount) {
        summary.totalUnits = summary.dwellingUnitCount
      }
      if (!summary.vacantUnits && summary.totalUnits && summary.totalUnits > summary.occupiedUnits) {
