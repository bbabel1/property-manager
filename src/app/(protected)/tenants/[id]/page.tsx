...
export default async function TenantDetailsPage({ params }: { params: Promise<{ id: string }> }) {
  ...
  if (Array.isArray(leaseContacts)) {
    ...
    for (const row of leaseRows) {
      const leaseArray = Array.isArray(row?.lease)
        ? row.lease
        : row?.lease
          ? [row.lease]
          : []
    ...
    <NavTabs defaultValue="summary" className="space-y-6 p-6">
      <div className="flex items-start justify-between">
        <div>
          ...
          <p className="text-sm text-muted-foreground">{primaryLeaseSubtitle}</p>
        </div>
      </div>
        ...
        <div className="grid grid-cols-1 gap-6 lg:grid-cols-3">
          ...
          <div>
            <div className="rounded-lg border border-primary/30 bg-primary/5 p-4 text-sm">
              <div className="mb-1 font-medium text-foreground">{primaryLeaseHeadline}</div>
              {primaryLease ? (
                <>
                  ...
                  <div className="space-y-2">
                    <div className="flex items-center justify-between">
                      ...
                      <span className="font-semibold text-foreground">{fmtUsd(0)}</span>
                    </div>
                    <div className="flex items-center justify-between">
                  ...
                  <p className="mt-3 text-muted-foreground">
                    Payment is due on the 1st of the month. If payment isn't received, a one-time fee of $50.00 will be charged on the 2nd of each
                    month. An additional daily fee of $10.00 will be charged starting on the 3rd and continue until the month ends. Late fees will
                    never exceed $100.00 per month.
                  </p>
                    ...
                    {leases.length === 0 ? (
                      ...
                      leases.map((l) => (
                        <TableRow key={l.id} className="hover:bg-muted/60">
                          ...
                          <TableCell className="text-sm">{l.unitName || l.propertyUnit || '—'}</TableCell>
