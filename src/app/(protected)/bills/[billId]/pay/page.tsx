...
export default async function PayBillPage({ params }: { params: Promise<{ billId: string }> }) {
  ...
  const lines = Array.isArray(linesRes?.data) ? (linesRes.data as TransactionLineWithProperty[]) : []
  ...
  for (const line of lines) {
    ...
    if (!propertyMeta.has(propertyId)) {
      const property = line?.properties as
        | {
            id?: string | null
            name?: string | null
            operating_bank_account_id?: string | null
  ...
  const primaryPropertyId =
    [...propertyTotals.entries()].sort((a, b) => b[1] - a[1])[0]?.[0] ?? [...propertyMeta.keys()][0] ?? null
  ...
  const bankAccounts = bankAccountsRaw.map((row) => {
    ...
    const lastFour = accountNumber && accountNumber.length > 4 ? accountNumber.slice(-4) : accountNumber
