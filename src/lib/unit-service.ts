...
export default class UnitService {
  ...
  static async listFromBuildium(params?: {
    ...
    const items = (res.json ?? []) as BuildiumUnit[]
  ...
  static async getFromBuildium(id: number, persist = false, orgId?: string): Promise<BuildiumUnit | null> {
    ...
    const item = (res.json ?? {}) as BuildiumUnit
  ...
  static async createInBuildiumAndDB(local: UnitRow): Promise<{ buildium: BuildiumUnit; localId: string }> {
    ...
    const res = await buildiumFetch('POST', '/rentals/units', undefined, payload, local.org_id)
    if (!res.ok) throw new Error(`Buildium unit create failed: ${res.status}`)
    const created = (res.json ?? {}) as BuildiumUnit
  ...
  static async updateInBuildiumAndDB(buildiumId: number, partial: Partial<UnitRow>, orgId?: string): Promise<BuildiumUnit> {
    ...
    const res = await buildiumFetch('PUT', `/rentals/units/${buildiumId}`, undefined, payload, orgId ?? partial.org_id)
    if (!res.ok) throw new Error(`Buildium unit update failed: ${res.status}`)
    const updated = (res.json ?? {}) as BuildiumUnit
  ...
  static async listImages(unitId: number, params?: { limit?: number; offset?: number }, orgId?: string): Promise<BuildiumUnitImage[]> {
    ...
    if (params?.limit) queryParams.limit = String(params.limit)
    if (params?.offset) queryParams.offset = String(params.offset)
    const res = await buildiumFetch('GET', `/rentals/units/${unitId}/images`, queryParams, undefined, orgId)
  ...
  static async uploadImage(
    ...
    if (!ticketRes.ok) {
      const errorText = typeof ticketRes.json === 'string' ? ticketRes.json : JSON.stringify(ticketRes.json ?? {})
      throw new Error(`Buildium unit image upload (metadata) failed: ${ticketRes.status} ${errorText}`)
    }

    const ticket = (ticketRes.json ?? {}) as { BucketUrl?: string; FormData?: Record<string, string>; PhysicalFileName?: string }
    ...
    const locateUploadedImage = async (): Promise<BuildiumUnitImage | null> => {
      ...
      for (let i = 0; i < attempts; i++) {
        ...
        if (!listRes.ok) {
          const errorText = typeof listRes.json === 'string' ? listRes.json : JSON.stringify(listRes.json ?? {})
          throw new Error(`Buildium unit image fetch failed after upload: ${listRes.status} ${errorText}`)
        }
        const images = (listRes.json ?? []) as BuildiumUnitImage[]
  ...
  static async updateImage(unitId: number, imageId: number, payload: { Description?: string }, orgId?: string): Promise<BuildiumUnitImage> {
    ...
    const img = (res.json ?? {}) as BuildiumUnitImage
  ...
  static async downloadImage(unitId: number, imageId: number, orgId?: string): Promise<BuildiumFileDownloadMessage> {
    ...
    return (res.json ?? {}) as BuildiumFileDownloadMessage
  }
  ...
  static async listNotes(unitId: number, params?: { limit?: number; offset?: number }, orgId?: string): Promise<BuildiumUnitNote[]> {
    ...
    if (params?.limit) queryParams.limit = String(params.limit)
    if (params?.offset) queryParams.offset = String(params.offset)
    const res = await buildiumFetch('GET', `/rentals/units/${unitId}/notes`, queryParams, undefined, orgId)
  ...
  static async getNote(unitId: number, noteId: number, orgId?: string): Promise<BuildiumUnitNote> {
    ...
    return (res.json ?? {}) as BuildiumUnitNote
  }
