...
type CriteriaRowConfig = {
  table: 'building' | 'property' | 'unit' | 'asset';
  field: string;
  operator:
    | 'equals'
    | 'not_equals'
...
export function sanitizeProgramCriteria(input: unknown): ComplianceProgramCriteria | null {
      ...
      const groups: string[] = Array.from(
        new Set(
          propertyFilters.occupancy_groups
            .map((g: unknown) => (typeof g === 'string' ? g.trim() : null))
            .filter((g: unknown): g is string => Boolean(g)),
        ),
      );
  ...
  if (assetFilters && typeof assetFilters === 'object') {
    ...
    if (Array.isArray(assetFilters.device_categories)) {
      const deviceCategories: ComplianceDeviceCategory[] = Array.from(
        new Set(
          assetFilters.device_categories
            .map((d: unknown) => normalizeDeviceCategoryValue(d))
            .filter((d: unknown): d is ComplianceDeviceCategory => Boolean(d)),
        ),
      );
      if (deviceCategories.length) af.device_categories = deviceCategories;
    }
    if (Array.isArray(assetFilters.exclude_device_categories)) {
      const excluded: ComplianceDeviceCategory[] = Array.from(
        new Set(
    ...
    if (Array.isArray(assetFilters.device_technologies)) {
      const technologies: string[] = Array.from(
        new Set(
          assetFilters.device_technologies
            .map((t: unknown) =>
              typeof t === 'string' ? matchDeviceTechnology(String(t)) || t.trim() : null,
            )
    ...
    if (typeof assetFilters.pressure_type === 'string') {
      const norm = normalizePressureType(assetFilters.pressure_type);
      if (norm) af.pressure_type = norm;
    }
...
const normalizeCriteriaRows = (input: unknown): CriteriaRowConfig[] => {
  ...
  return input
    .map((row) => {
      ...
      const connector =
        raw.connector === 'OR' || raw.connector === 'AND' ? (raw.connector as LogicalConnector) : undefined;
      return {
        table: table as CriteriaRowConfig['table'],
        field,
        operator: operator as CriteriaRowConfig['operator'],
        compareValue: typeof raw.compareValue === 'string' ? raw.compareValue : '',
        compareValueMin: typeof raw.compareValueMin === 'string' ? raw.compareValueMin : '',
        compareValueMax: typeof raw.compareValueMax === 'string' ? raw.compareValueMax : '',
        connector,
      };
    })
...
const evaluateRow = (
  ...
  const compare = row.compareValue || '';
  const compareMin = row.compareValueMin || '';
  const compareMax = row.compareValueMax || '';

  switch (row.operator) {
    case 'equals':
      return stringValue(value).toLowerCase() === compare.toLowerCase();
    case 'not_equals':
      return stringValue(value).toLowerCase() !== compare.toLowerCase();
    case 'contains':
      return stringValue(value).toLowerCase().includes(compare.toLowerCase());
    case 'not_contains':
      return !stringValue(value).toLowerCase().includes(compare.toLowerCase());
    case 'starts_with':
      return stringValue(value).toLowerCase().startsWith(compare.toLowerCase());
    case 'ends_with':
      return stringValue(value).toLowerCase().endsWith(compare.toLowerCase());
    case 'gt': {
      if (!isSet(value) || !isSet(compare)) return false;
    ...
    case 'between': {
      if (!isSet(value) || !isSet(compareMin) || !isSet(compareMax)) return false;
      const num = Number(value);
      const min = Number(compareMin);
      const max = Number(compareMax);
...
const evaluateCriteriaRows = (
  rows: CriteriaRowConfig[],
  property?: PropertyMeta | null,
  asset?: AssetMeta | null,
): boolean => {
  ...
  rows.forEach((row) => {
    const rowResult = evaluateRow(row, property, asset);
    if (result === null) {
      result = rowResult;
    } else if (row.connector === 'OR') {
...
export function propertyMatchesCriteria(
  criteria: ComplianceProgramCriteria | null | undefined,
  property?: PropertyMeta | null,
): boolean {
  ...
  if (occupancy_groups && occupancy_groups.length > 0) {
    if (!property.occupancy_group || !occupancy_groups.includes(property.occupancy_group))
      return false;
  }
  ...
  const dwellingCount = property.residential_units ?? null;
...
export function assetMatchesCriteria(
  ...
  if (pressure_type) {
    const normPressure =
      typeof pressure_type === 'string' ? normalizePressureType(pressure_type) : null;
    if (normPressure && devicePressure !== normPressure) return false;
  }
  return true;
}

export function programTargetsProperty(
  program: Pick<ComplianceProgram, 'applies_to' | 'criteria' | 'override_fields'>,
  property: PropertyMeta | null | undefined,
): boolean {
  const rows = getCriteriaRowsForProgram(program);
  if (rows.length > 0) {
    return evaluateCriteriaRows(rows, property, null);
  }
  return propertyMatchesCriteria(program.criteria, property);
}

export function programTargetsAsset(
  program: Pick<ComplianceProgram, 'applies_to' | 'criteria' | 'override_fields'>,
  asset: AssetMeta | null | undefined,
  property: PropertyMeta | null | undefined,
): boolean {
  const rows = getCriteriaRowsForProgram(program);
  if (rows.length > 0) {
    const hasAssetScope = rows.some((row) => row.table === 'asset' || row.table === 'unit');
    if (!hasAssetScope) return false;
    return evaluateCriteriaRows(rows, property, asset);
  }
  if (!propertyMatchesCriteria(program.criteria, property)) return false;
  if (!assetMatchesCriteria(program.criteria, asset)) return false;
  return true;
}
...
function matchDeviceCategory(value: string): ComplianceDeviceCategory | null {
  ...
  if (normalized.includes('pneumatic') || normalized.includes('vacuum'))
    return 'pneumatic_elevator';
...
function matchDeviceTechnology(value: string): string | null {
  ...
  if (
    ...
    normalized.includes('gearless') ||
    normalized.includes('geared')
  )
    return 'traction';
...
function normalizePressureType(value: unknown): 'low_pressure' | 'high_pressure' | null {
  if (!value) return null;
  const str = String(value).toLowerCase();
