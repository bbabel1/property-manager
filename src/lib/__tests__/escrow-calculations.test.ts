...
vi.mock('@/lib/db', () => {
  ...
  type Filter = { column: string; value: any; type: 'eq' | 'lte' | 'gte' | 'in' };

  const getValue = (row: Row, path: string) =>
    path.split('.').reduce((acc: any, key) => (acc ? acc[key] : undefined), row);
  ...
  class QueryBuilder implements PromiseLike<{ data: Row[]; error: null }> {
    ...
    private applyFilters(): Row[] {
      if (this.operation === 'insert') {
        const nextRows = (this.insertRows ?? []).map((row) => ({
          id: row.id ?? `${this.table}-${Math.random().toString(16).slice(2)}`,
          ...row,
        }));
      ...
      const rows = [...((tableData as Record<string, Row[]>)[this.table] ?? [])];
    ...
    then<TResult1 = { data: Row[]; error: null }, TResult2 = never>(
      onfulfilled?: ((value: { data: Row[]; error: null }) => TResult1 | PromiseLike<TResult1>) | undefined | null,
  ...
  return { supabaseAdmin, __setTableData, __getTableData };
});
...
beforeAll(async () => {
  ...
  setTableData = (db as any).__setTableData as (tables: EscrowTableData) => void;
  getTableData = (db as any).__getTableData as () => EscrowTableData;
});
