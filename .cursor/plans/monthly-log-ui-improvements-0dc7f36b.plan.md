...
export async function getEscrowBalance(unitId: string, upToDate: string): Promise<{
  deposits: number;
  withdrawals: number;
  balance: number;
...
*(Same as original - transaction_lines with processing fee GL account)*
...
*(Same as original, with added note for Escrow stage)*
...
**Flow**:
*(Same as original, with added deliverability checks)*
