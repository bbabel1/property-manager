...
Use this after mapper changes or Buildium API schema changes to ensure payments/deposits still map correctly (header fields, line metadata, and splits).
...
4. If the production data path is affected, rerun the backfill or targeted upsert for impacted Buildium transaction IDs.
