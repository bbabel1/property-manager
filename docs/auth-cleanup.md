...
1. Identify NULL org_ids on org-scoped tables (should be rare after backfills):
   ...
   - Remediate by deleting orphaned rows or backfilling with the correct org_id; enforce NOT NULL if possible after
     cleanup.
...
2. Normalize AppRole strings in membership tables:
   ...
   - Validate roles against the AppRole set (`platform_admin, org_admin, org_manager, org_staff, owner_portal,
     tenant_portal, vendor_portal`) and quarantine unknowns.
