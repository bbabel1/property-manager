...
## Current Snapshot
- Next.js app lives in `src/app`; APIs under `src/app/api`.
- Domain logic already partially lives in `src/modules/{banking,leases,monthly-logs,owners,properties,staff,units}` but UI for those domains is spread across `src/components/*` and several root-level `src/*.tsx` modals (e.g., `src/AddUnitModal.tsx`).
...
- `src/components` is large and mixes domain-specific pages (leases, monthly-logs, reconciliations, properties, transactions) with common widgets and UI primitives.
- Scripts: `scripts/` has subfolders (buildium/, database/, cron/, maintenance/, migrations/, setup/, utils/, workflows/), but many root-level diagnostic/maintenance scripts sit alongside them.
- Docs: extensive under `docs/`; status/summary markdowns now live in `docs/reports/` (e.g., `MIGRATION_READY_CHECKLIST.md`, `MCP_CLEANUP_SUMMARY.md`, `MONTHLY_LOG_*`, `typecheck-report.md`).
- Stories: `stories/` at root; tests: `tests/` only contains `README.md` (helpers and specs elsewhere).
- Supabase: migrations/functions/config in `supabase/`; styles/assets in `styles/` and `public/`.

## Target Layout (end-state)
- `/docs`: keep architecture/feature/database docs; add `docs/reports/` for status logs and summaries; keep Storybook docs either under `stories/` or `docs/ui`.
- `/src/app`: Next.js routes and API handlers.
- `/src/modules/{domain}`: co-locate domain UI + server handlers + domain hooks (properties, owners, leases, monthly-logs, banking, units, staff). Prefer using module-level subfolders (e.g., `monthly-logs/components`, `monthly-logs/server`).
...
- `/scripts`: group by function: `scripts/migrations/`, `scripts/cron/`, `scripts/smoke/`, `scripts/diagnostics/` (root `check-*`, `verify-*`, `debug-*`, `fix-*`), keep `scripts/buildium/`, `scripts/database/`, `scripts/utils/`, `scripts/sql/`, `scripts/setup/`.
- `/tests`: add `tests/helpers/` for shared fixtures/mocks; keep e2e/unit/visual under `tests/{e2e,unit,visual}` when they exist.
...
3) **Domain consolidation**
   ...
   - Move root-level domain modals (e.g., `src/AddUnitModal.tsx`, `src/EditPropertyModal.tsx`, `src/CreateOwnerModal.tsx`, address autocomplete variants) into their domain module folders; export from module index if needed.
...
5) **Tests and verification**
   - Add `tests/helpers/` for shared fixtures as they are discovered; relocate any stray test helpers from root.
...
- Duplicate modals: `src/*Modal.tsx` and autocomplete components → corresponding `src/modules/{domain}/components/`.
