...
- **Next.js route** (broad coverage):
  ...
  - **Lease transactions**: `upsertLeaseTransactionWithLinesLocal`; double-entry integrity enforced; deletes remove transaction + lines.
    - **Bank/Undeposited Funds precedence (Payment/ApplyDeposit without Buildium bank GL)**: prefer Buildium-provided bank GL, else org-scoped **Undeposited Funds**, else property operating/deposit-trust as a last resort. Persist the final `transactions.bank_gl_account_id`.
