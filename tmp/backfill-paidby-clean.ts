...
async function run(){
  ...
  let propIds = new Set<string>();
  let unitIds = new Set<string>();
  if (buildiumIds.length > 0) {
    const { data: payments, error: payErr } = await supabaseAdmin
      .from('transactions')
      .select('tenant_id, paid_to_tenant_id, transaction_lines(property_id, unit_id)')
      .in('buildium_transaction_id', buildiumIds);
    if (payErr) throw payErr;
    (payments || []).forEach((p:any)=>{
      if (p?.tenant_id) tenantIds.add(String(p.tenant_id));
      if (p?.paid_to_tenant_id) tenantIds.add(String(p.paid_to_tenant_id));
      (p?.transaction_lines || []).forEach((l:any)=>{
